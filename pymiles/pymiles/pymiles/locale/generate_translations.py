""" 
 @file
 @brief This file updates the pymiles.POT (language translation template) by scanning all source files.
 @author Jonathan Thomas <jonathan@pymiles.org>
 
 This file helps you generate the POT file that contains all of the translatable
 strings / text in pymiles.  Because some of our text is in custom XML files,
 ...
 Command to create the individual language PO files (Ascii files)
		$ msginit --input=pymiles.pot --locale=fr_FR
		$ msginit --input=pymiles.pot --locale=es

 Command to update the PO files (if text is added or changed)
		$ msgmerge en_US.po pymiles.pot -U
		$ msgmerge es.po pymiles.pot -U

 Command to compile the Ascii PO files into binary MO files
		$ msgfmt en_US.po --output-file=en_US/LC_MESSAGES/pymiles.mo
		$ msgfmt es.po --output-file=es/LC_MESSAGES/pymiles.mo

 Command to compile all PO files in a folder
		$ find -iname "*.po" -exec msgfmt {} -o {}.mo \;

 Command to combine the 2 pot files into 1 file
       $ msgcat ~/pymiles/locale/pymiles/pymiles_source.pot ~/pymiles/pymiles/locale/pymiles/pymiles_glade.pot -o ~/pymiles/main/locale/pymiles/pymiles.pot

 @section LICENSE
 
 
 You should have received a copy of the GNU General Public License
 along with pymiles Library.  If not, see <http://www.gnu.org/licenses/>.
 """
...
# get the path of the main pymiles folder
langage_folder_path = os.path.dirname(os.path.abspath(__file__))
pymiles_path = os.path.dirname(langage_folder_path)
export_path = os.path.join(pymiles_path, 'presets')
windows_ui_path = os.path.join(pymiles_path, 'windows', 'ui')
locale_path = os.path.join(pymiles_path, 'locale')

log.info("-----------------------------------------------------")
log.info(" Creating temp POT files")
log.info("-----------------------------------------------------")

# create empty temp files in the /pymiles/language folder (these are used as temp POT files)
temp_files = ['pymiles_QtUi.pot',]
...
pymiles_path, os.path.join(langage_folder_path, 'pymiles_source.pot')), shell=True)
...
subprocess.call('lupdate *.ui -ts %s' % (os.path.join(langage_folder_path, 'pymiles_QtUi.ts')), shell=True)
subprocess.call('lupdate *.ui -ts %s' % (os.path.join(langage_folder_path, 'pymiles_QtUi.pot')), shell=True)
os.chdir(langage_folder_path)

# Rewrite the UI POT, removing msgctxt
output = open(os.path.join(langage_folder_path, "clean.po"), 'w')
for line in open(os.path.join(langage_folder_path, 'pymiles_QtUi.pot'), 'r'):
    if not line.startswith('msgctxt'):
        output.write(line)
# Overwrite original PO file
output.close()
shutil.copy(os.path.join(langage_folder_path, "clean.po"), os.path.join(langage_folder_path, 'pymiles_QtUi.pot'))
os.remove(os.path.join(langage_folder_path, "clean.po"))

# Remove duplciates (if any found)
subprocess.call('msguniq %s --use-first -o %s' % (os.path.join(langage_folder_path, 'pymiles_QtUi.pot'),
...
'''
temp_files = ['pymiles_source.pot', 'pymiles_glade.pot']
...
# Append on properties from libpymiles
objects = [pymiles.Clip(), pymiles.Blur(), pymiles.Brightness(),
           pymiles.ChromaKey(), pymiles.Deinterlace(), pymiles.Mask(),
           pymiles.Negate(), pymiles.Saturation()]

# Loop through each libpymiles object
for object in objects:
    ...
    for key in props.keys():
        ...
        if "name" in object.keys():
            effects_text[object["name"]] = "libpymiles (Clip Properties)"
        if "choices" in object.keys():
            for choice in object["choices"]:
                effects_text[choice["name"]] = "libpymiles (Clip Properties)"

# Append Effect Meta Data
e = pymiles.EffectInfo()
props = json.loads(e.Json())

# Loop through props
for effect in props:
    if "name" in effect:
        effects_text[effect["name"]] = "libpymiles (Effect Metadata)"
    if "description" in effect:
        effects_text[effect["description"]] = "libpymiles (Effect Metadata)"
...
# header of POT file
header_text = ""
header_text = header_text + '# pymiles Database Application Generator POT Template File.\n'
header_text = header_text + '# Copyright (C) 2015-2016 Ted Lazaros\n'
header_text = header_text + '# This file is distributed under the same license as pymiles.\n'
header_text = header_text + '# Ted Lazaros <tedlaz@gmail.com>, 2016.\n'
header_text = header_text + '#\n'
header_text = header_text + '#, fuzzy\n'
header_text = header_text + 'msgid ""\n'
header_text = header_text + 'msgstr ""\n'
header_text = header_text + '"Project-Id-Version: pymiles Database Application Generator (version: %s)\\n"\n' % info.VERSION
header_text = header_text + '"Report-Msgid-Bugs-To: Ted Lazaros <tedlaz@gmail.com>\\n"\n'
header_text = header_text + '"POT-Creation-Date: %s\\n"\n' % datetime.datetime.now()
header_text = header_text + '"PO-Revision-Date: YEAR-MO-DA HO:MI+ZONE\\n"\n'
header_text = header_text + '"Last-Translator: Ted Lazaros <tedlaz@gmail.com>\\n"\n'
...
temp_files = [['pymiles_effects.pot', effects_text], ['pymiles_export.pot', export_text],
              ['pymiles_transitions.pot', transitions_text]]
...
command = command + " -o " + os.path.join(locale_path, "pymiles.pot")
...
# get the entire text of pymiles.POT
f = open(os.path.join(locale_path, "pymiles.pot"), "r")
# read entire text of file
entire_source = f.read()
f.close()

# Create Final POT Output File
if os.path.exists(os.path.join(locale_path, "pymiles.pot")):
    os.remove(os.path.join(locale_path, "pymiles.pot"))
final = open(os.path.join(locale_path, "pymiles.pot"), "w")
...
temp_files = ['pymiles_source.pot', 'pymiles_glade.pot', 'pymiles_effects.pot', 'pymiles_export.pot',
              'pymiles_transitions.pot', 'pymiles_QtUi.pot', 'pymiles_QtUi.ts']
for temp_file_name in temp_files:
    temp_file_path = os.path.join(langage_folder_path, temp_file_name)
    if os.path.exists(temp_file_path):
        os.remove(temp_file_path)
...
# output success
log.info("-----------------------------------------------------")
log.info(" The /pymiles/locale/pymiles/pymiles.pot file has")
log.info(" been successfully created with all text in pymiles.")
log.info("-----------------------------------------------------")
