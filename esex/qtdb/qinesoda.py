...
class fInsertEggrafi(QtGui.QDialog):
    def __init__(self, dbt, parent=None):
        ...
        lbPar = QtGui.QLabel(u'Υποκατάστημα')
        glayout.addWidget(lbPar, 0, 2)
        self.yp = dbf.DbButtonLineEdit('SELECT * FROM yp',
                                       db=self.db,
                                       idVal=1,
                                       tbl='yp_id')
        ...
        lbSyn = QtGui.QLabel(u'Πελάτης')
        glayout.addWidget(lbSyn, 1, 2, 1, 1)
        self.pel = dbf.DbButtonLineEdit('SELECT id,afm,epon FROM pel',
                                        db=self.db,
                                        tbl='pel_id')
        glayout.addWidget(self.pel, 1, 3, 1, 1)

        lbSr = QtGui.QLabel(u'Σειρά')
        glayout.addWidget(lbSr, 2, 0, 1, 1)
        self.sr = dbf.DbButtonLineEdit('SELECT id,srp FROM sr',
                                       db=self.db,
                                       tbl='sr_id')
        self.sr.setMaximumWidth(100)
        glayout.addWidget(self.sr, 2, 1, 1, 1)
        lbPar = QtGui.QLabel(u'Παραστατικό')
        glayout.addWidget(lbPar, 2, 2, 1, 1)
        self.par = dbf.DbLineEdit()
        self.par.setMaximumWidth(150)
        glayout.addWidget(self.par, 2, 3, 1, 1)
        vlayout = QtGui.QVBoxLayout(self)
        vlayout.addWidget(dbf.makeTitle(u'Εγγραφές εσόδων (πωλήσεων)'))
    ...
    def makeConnections(self):
        self.connect(self.tbl,
                     QtCore.SIGNAL("cellChanged(int, int)"),
                     self.onCellChange)
        self.bSave.clicked.connect(self.dbSave)

    def dbSave(self):
        sqlm = "INSERT INTO es (yp_id,dat,sr_id,par,pel_id,tval,tfpa,ttot) VALUES (?,?,?,?,?,?,?,?)"
        sqld = "INSERT INTO esd (es_id,et_id,val,fpa) VALUES (?,?,?,?)"
        imnia = '%s' % self.dat.date().toString('yyyy-MM-dd')
        ypok = '%s' % self.yp.timi
        pel = '%s' % self.pel.timi
        seira = '%s' % self.sr.timi
        ...
        ttot = '%s' % sumv[3]
        validated = u''
        if not dbf.allowInsertDate(imnia, self.db):
            validated += u'Η ημερομηνία ανήκει σε κλειστή περίοδο\n'
        if tval == '0':
            validated += u'Τουλάχιστον μία γραμμή με αξία\n'
        if pel == 'None':
            validated += u'Στοιχεία πελάτη\n'
        if seira == 'None':
            validated += u'Σειρά παραστατικού\n'
        if parko == '':
            validated += u'Αριθμός παραστατικού\n'
        for lin in self.arData:
            if lin[1] == 0:
                continue
            etid = '%s' % lin[0]
            if etid == '0':
                validated += u'Λογαριασμός\n'
        if len(validated) > 0:
            QtGui.QMessageBox.critical(self,
                                       u"Συμπληρώστε τα παρακάτω",
                                       validated)
            return
        # print ypok,imnia,seira,parko,pel
        esid = dbf.commitToDb(sqlm,
                              [ypok, imnia, seira, parko, pel, tval, tfpa, ttot],
                              self.db)
        for lin in self.arData:
            if lin[1] == 0:
                continue
            ...
            val = '%s' % lin[1]
            fpa = '%s' % lin[2]
            k = dbf.commitToDb(sqld, [esid, etid, val, fpa], self.db)
        if esid:
            logData = u"Κατεχωρήθη στα έσοδα το %sΝο.%s(%s) αξίας %s ευρώ με AA : %s\n" % (self.sr.text(), parko, imnia, ttot, esid)
            self.addLog(logData, True)
        else:
            logData = u"Το παραστατικό %sNo.%s είναι ήδη καταχωρημένο\n" % (self.sr.text(), parko)
    ...
    def resetData(self):
        ...
        if dbf.isNum(self.par.text()):
            newPar = '%s' % (int(self.par.text()) + 1)
            self.par.setText(newPar)
        else:
            self.par.setText('')
    ...
    def setNewLmoLines(self):
        dat1, heads = dbf.getDbRows('SELECT * FROM et', self.db)
        for frm in dat1:
            self.newLine()
    ...
    def onCellChange(self, row, col):
        ...
        if col == 0:
            ...
            if bb != self.aa:
                frm = dbf.fFind('SELECT * FROM et', self.db, 'et', 1, parent=self)
            ...
            txtv = self.tbl.item(row, col).text().replace(',', '.')
            val = dbf.dec(txtv)
    ...
    def findCustomerByAFM(self, txt):
        txtb = txt+'%'
        sql = "SELECT id, afm,epon FROM pel WHERE afm LIKE '%s'" % txtb
        ...
        if pel:
            ...
            if len(txt) == 9:
                ...
                if pel['valid']:
                    ...
                    epon = '%s' % pel['name']
                    addr = '%s' % pel['address']
                    sqin = "INSERT INTO pel (ccod,afm,rdat,epon,addr) VALUES (?, ?, ?, ?, ?)"
                    msgtxt = u'Βρέθηκε πελάτης με στοιχεία:\n ΑΦΜ : %s\nΕπωνυμία : %s\nΔιεύθυνση : %s\n\nΝα καταχωρηθεί ??' % (afm,epon,addr)
                    reply = dbf.grMsgBox.question(self, u'Επιτυχής αναζήτηση',msgtxt , QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
                    if reply == QtGui.QMessageBox.Yes:
                        synid = dbf.commitToDb(sqin,
                                               [ccod, afm, rdat, epon, addr],
                                               self.db)
                        self.pel.setValue(synid)
                        self.addLog(u'Έγινε καταχώρηση Πελάτη: %s' % self.pel.text(), True)
                else:
                    if 'conError' in pel:
                        QtGui.QMessageBox.critical(self,
                                                   u"Αποτυχία σύνδεσης",
                                                   u"Δεν γίνεται σύνδεση με τη Βάση Δεδομένων του VIES")
                    else:
                        QtGui.QMessageBox.critical(self,
                                                   u"Αποτυχία αναζήτησης",
                                                   u"Δεν βρέθηκε ενεργό ΑΦΜ : %s στη Βάση Δεδομένων του VIES" % txt)
            else:
                self.addLog(u'Δώσε ολόκληρο το ΑΦΜ για να γίνει αναζήτηση στη βάση δεδομένων του VIES')
