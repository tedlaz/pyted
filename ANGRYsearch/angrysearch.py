...
class Thread_database_update(Qc.QThread):
    ...
    def __init__(self, dirs_excluded, root_dir='/', parent=None):
        super().__init__()
        self.table = []
        self.prep_excluded = []
        self.crawl_time = ''
        self.database_time = ''
        print(root_dir)
        self.root_dir = root_dir.encode('UTF-8')
    ...
    def crawling_drives(self):
        def error(err):
            print(err)
        rpath = '/run/media/tedlaz/wd1terra'
        # root_dir = b'/run/media/tedlaz/wd1terra/' #  b'/'
        tstart = datetime.now()
        ...
        for root, dirs, files in modul_var.walk(self.root_dir, onerror=error):
            dirs.sort()
            files.sort()
            ...
            for fname in files:
                ...
                try:
                    ...
                    size = 0
                    epoch_time = 0
                # fpath = utf_path.replace(rpath, '')
                file_list.append(('0', utf_path, size, epoch_time))
        # Αν θέλουμε και φακέλλους προσθέτουμε και το dir_list
        self.table = file_list  # dir_list + file_list
...
class Custom_table_model(Qc.QAbstractTableModel):
    def __init__(self, table_data=[[]], parent=None):
        ...
        self.sort_ed = False
        self.headers = ['Όνομα', 'Διαδρομή', 'Μέγεθος', 'Ημ/νία αλλαγής']
...
class Center_widget(Qw.QWidget):
    ...
    def initUI(self):
        ...
        self.table = My_table_view(self.set['row_height'])
        self.upd_button = Qw.QPushButton('Ενημέρωση')
        ...
        grid.addWidget(self.upd_button, 1, 3)
...
class Gui_MainWindow(Qw.QMainWindow):
    ...
    def read_settings(self):
        ...
        self.read_qsettings_item('directories_included', 'str')
    ...
    def closeEvent(self, event):
        ...
        self.setset('fast_search_but_no_substring', True)
        self.setset('typing_delay', False)
        self.setset('darktheme', True)
        self.setset('fm_path_doubleclick_selects', True)
        self.setset('icon_theme', 'adwaita')
        self.setset('file_manager', '')
        self.setset('row_height', 0)
        self.setset('number_of_results', 500)
        self.setset('directories_included', '/')
        self.setset('directories_excluded',
            '/bin /boot /dev /lib /lib64 /opt /proc /root /sys /usr')
        self.setset('conditional_mounts_for_autoupdate', '')
        self.setset('notifications', True)
        self.setset('regex_mode', False)
    ...
    def init_GUI(self):
        ...
        if self.set['darktheme'] is True:
            ...
            if os.path.isfile('qdarkstylesheet.qss'):
                qssf = 'qdarkstylesheet.qss'
            elif os.path.isfile('/usr/share/angrysearch/qdarkstylesheet.qss'):
                qssf = '/usr/share/angrysearch/qdarkstylesheet.qss'
            elif os.path.isfile('/opt/angrysearch/qdarkstylesheet.qss'):
                qssf = '/opt/angrysearch/qdarkstylesheet.qss'

            if qssf:
                with open(qssf, 'r') as afile:
    ...
    def make_sys_tray(self):
        if Qw.QSystemTrayIcon.isSystemTrayAvailable():
            ...
            menu.addSeparator()
            exitAction = menu.addAction('Έξοδος')
...
class Update_dialog_window(Qw.QDialog):
    ...
    def initUI(self):
        self.setWindowTitle('Ανανέωση βάσης')

        self.includ_dirs = self.parent().set['directories_included']
        self.exclud_dirs = ' '.join(self.parent().set['directories_excluded'])
        combobox_text = self.parent().set['icon_theme']

        self.icon_theme_label = Qw.QLabel('Θέμα εικονιδίων:')
        ...
        self.included_label = Qw.QLabel('Αρχικό σημείο αναζήτησης:')
        self.included_dirs_btn = Qw.QPushButton(self.includ_dirs)
        self.excluded_label = Qw.QLabel('Φάκελλοι που θα αγνοηθούν:')
        self.excluded_dirs_btn = Qw.QPushButton(self.exclud_dirs)
        self.crawl0_label = Qw.QLabel('πρόοδος:')
        self.crawl_label = Qw.QLabel('')
        self.label_1 = Qw.QLabel('• αναζήτηση αρχείων')
        self.label_2 = Qw.QLabel('• Δημιουργία νέας βάσης')
        self.label_3 = Qw.QLabel('• Αντικατάσταση παλιάς βάσης')
        self.OK_button = Qw.QPushButton('Ανανέωση')
        self.cancel_button = Qw.QPushButton('Ακύρωση')

        if self.includ_dirs == '':
            self.included_dirs_btn.setText('none')
            self.included_dirs_btn.setStyleSheet('color:#888;font: italic;')
        ...
        self.included_dirs_btn.clicked.connect(self.include_dialog)
        ...
        grid.addWidget(self.included_label, 1, 0)
        grid.addWidget(self.included_dirs_btn, 1, 1)
        grid.addWidget(self.excluded_label, 2, 0)
        grid.addWidget(self.excluded_dirs_btn, 2, 1)
        grid.addWidget(self.crawl0_label, 3, 0)
        grid.addWidget(self.crawl_label, 3, 1)
        grid.addWidget(self.label_1, 4, 0, 1, 2)
        grid.addWidget(self.label_2, 5, 0, 1, 2)
        grid.addWidget(self.label_3, 6, 0, 1, 2)
    ...
    def include_dialog(self):
        text = str(Qw.QFileDialog.getExistingDirectory(self,
                                                       u'Επιλογή φακέλλου',
                                                       '/',
        ...
        if not text:
            return
        text = text.strip()
        self.includ_dirs = text
        self.settings.setValue('directories_included', text)
        self.parent().set['directories_included'] = text
        if text == '':
            self.included_dirs_btn.setText('none')
            self.included_dirs_btn.setStyleSheet('color:#888;'
                                                 'font:italic;')
        else:
            self.included_dirs_btn.setText(text)
            self.included_dirs_btn.setStyleSheet('')
        self.OK_button.setFocus()
