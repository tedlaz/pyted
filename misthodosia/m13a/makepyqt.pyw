...
import stat
import sys
from PyQt4.QtCore import (PYQT_VERSION_STR, QDir, QFile, QProcess,
        QSettings, QT_VERSION_STR, QVariant, Qt, SIGNAL)
from PyQt4.QtGui import (QApplication, QCheckBox, QCursor, QDialog,
...
class OptionsForm(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.pyuic4Label = QLabel(settings.value("pyuic4", QVariant(PYUIC4)).toString())
        self.pyuic4Label.setFrameStyle(QFrame.StyledPanel| QFrame.Sunken)
        pyuic4Button = QPushButton("py&uic4...")
        pyrcc4Label = QLabel("pyrcc4")
        self.pyrcc4Label = QLabel(settings.value("pyrcc4", QVariant(PYRCC4)).toString())
        ...
        self.pylupdate4Label = QLabel(settings.value("pylupdate4", QVariant(PYLUPDATE4)).toString())
        self.pylupdate4Label.setFrameStyle(QFrame.StyledPanel| QFrame.Sunken)
        pylupdate4Button = QPushButton("py&lupdate4...")
        lreleaseLabel = QLabel("lrelease")
        self.lreleaseLabel = QLabel(settings.value("lrelease", QVariant("lrelease")).toString())
        ...
        self.connect(pyuic4Button, SIGNAL("clicked()"), lambda: self.setPath("pyuic4"))
        self.connect(pyrcc4Button, SIGNAL("clicked()"), lambda: self.setPath("pyrcc4"))
        self.connect(pylupdate4Button, SIGNAL("clicked()"), lambda: self.setPath("pylupdate4"))
        self.connect(lreleaseButton, SIGNAL("clicked()"), lambda: self.setPath("lrelease"))
    ...
    def accept(self):
        ...
        settings.setValue("pylupdate4", QVariant(self.pylupdate4Label.text()))
...
class Form(QMainWindow):
    ...
    def __init__(self):
        ...
        settings = QSettings()
        rememberPath = settings.value("rememberpath", QVariant(True if sys.platform.startswith("win") else False)).toBool()
        if rememberPath:
    ...
    def closeEvent(self, event):
        settings = QSettings()
        settings.setValue("rememberpath", QVariant(self.rememberPathAction.isChecked()))
    ...
    def _build(self, path):
        settings = QSettings()
        pyuic4 = unicode(settings.value("pyuic4", QVariant(PYUIC4)).toString())
        pyrcc4 = unicode(settings.value("pyrcc4", QVariant(PYRCC4)).toString())
                   ...
                   os.stat(target)[stat.ST_MTIME]):
                    ...
                    if self.debugCheckBox.isChecked():
                        ...
                        if (not process.waitForFinished(2 * 60 * 1000) or
                            ...
                            msg = ("<font color=red>FAILED: {0}</font>"
                                   .format(command))
                            failed += 1
    ...
    def _clean(self, path):
        ...
        for name in os.listdir(path):
            ...
            source = None
            if (target.endswith(".py") or target.endswith(".pyc") or target.endswith(".pyo")):
                ...
                if source is not None:
                    if os.access(source, os.F_OK):
                        if self.debugCheckBox.isChecked():
                            self.logBrowser.append("<font color=green># delete {0}</font>".format(target.replace(prefix, "")))
                        else:
                            deletelist.append(target)
                    else:
                        self.logBrowser.append("<font color=darkred>will not remove '{0}' since `{1}' not found</font>".format(target.replace(prefix, ""), source.replace(prefix, "")))
        if not self.debugCheckBox.isChecked():
            for target in deletelist:
                self.logBrowser.append("deleted <font color=red>{0}</font>".format(target.replace(prefix, "")))
                os.remove(target)
                QApplication.processEvents()
    ...
    def _translate(self, path):
        ...
        if not tsfiles:
            return
        settings = QSettings()
        pylupdate4 = unicode(settings.value("pylupdate4", QVariant(PYLUPDATE4)).toString())
        lrelease = unicode(settings.value("lrelease", QVariant(LRELEASE)).toString())
        process = QProcess()
        failed = 0
        for ts in tsfiles:
            qm = ts[:-3] + ".qm"
            ...
            msg = "updated <font color=blue>{0}</font>".format(ts.replace(prefix, ""))
            if self.debugCheckBox.isChecked():
                msg = "<font color=green># {0}</font>".format(msg)
            else:
                process.start(command1, args1)
                if not process.waitForFinished(2 * 60 * 1000):
                    msg = "<font color=red>FAILED: {0}</font>".format(command1)
                    failed += 1
            self.logBrowser.append(msg)
            msg = "generated <font color=blue>{0}</font>".format(qm.replace(prefix, ""))
            if self.debugCheckBox.isChecked():
                msg = "<font color=green># {0}</font>".format(msg)
            else:
                process.start(command2, args2)
                if not process.waitForFinished(2 * 60 * 1000):
                    msg = "<font color=red>FAILED: {0}</font>".format(command2)
                    failed += 1
            self.logBrowser.append(msg)
            QApplication.processEvents()
        if failed:
...
if sys.platform.startswith("win"):
    PATH = os.path.join(os.path.dirname(sys.executable),"Lib/site-packages/PyQt4/bin")
