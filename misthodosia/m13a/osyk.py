...
import zipfile
import os, sys
import osykGetFromInternet
'''
osykfile = os.path.join(os.path.dirname(__file__), 'osyk.zip')
doyFile  = os.path.join(os.path.dirname(__file__), 'ddoy.txt')
ikaFile  = os.path.join(os.path.dirname(__file__), 'dika.txt')
newDbFile = os.path.join(os.path.dirname(__file__), 'dnewDb.sql') 
''' 
osykfile = os.path.join(sys.path[0],'osyk.zip').replace('\\','/')
doyFile  = os.path.join(sys.path[0],'ddoy.txt').replace('\\','/')
ikaFile  = os.path.join(sys.path[0],'dika.txt').replace('\\','/')
newDbFile = os.path.join(sys.path[0],'dnewDb.sql').replace('\\','/') 
...
if __name__ == "__main__":
	...
	if not os.path.exists(osykfile):
		...
		localfile = 'osyk.zip'
		osykGetFromInternet.download(urlfile,localfile)
