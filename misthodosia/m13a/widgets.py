...
import res_rc 
import sql_metadata as fsql
import widgets2 as qtutils
'''
...
class DbDateEdit(QtGui.QDateEdit):
    def __init__(self, sqliteDate=None, parent=None):
        ...
        self.setVal(sqliteDate)
            
    def setVal(self,sqliteDate):   
        if sqliteDate:
...
class ButtonLineEdit(QtGui.QLineEdit):
    ...
    def __init__(self, sql=None, titles=None,db=None,idVal=None, sql1=None,sizes=[30,300], parent=None):
        ...
        self.sql    = sql
        self.sql1   = sql1 # SQL ειδικά για το setVal ...
        ...
        self.button.setIcon(QtGui.QIcon(":/images/icon_find.png"))
    ...
    def setVal(self,idVal):
        ...
        v = utils_db.getDbOneRow(sql, self.db)
...
class WeekDays(QtGui.QWidget):
    ...
    def __init__(self,vals=[1,1,1,1,1,0,0],parent=None,dayNames=u'ΔΕ ΤΡ ΤΕ ΠΕ ΠΑ ΣΑ ΚΥ'):
        ...
        self.setVal(vals) # Δημιουργεί το self.grid
        self.selected = [0, 0]
    ...
    def getVal(self,strVal=False): # strval : Εάν η επιστρεφόμενη τιμή είναι string ή array
        if strVal:
            st = '['
            for i in range(7):
                if i == 6:
    ...
    def str_(self):
        return self.getVal(True)
        
    def setVal(self,darr=[0,0,0,0,0,0,0]):
    ...
    def reset(self):
        ...
        self.setVal([0,0,0,0,0,0,0])
        
    def set5days(self):
        'Set Standard five days week'
        self.setVal([1,1,1,1,1,0,0])
...
class autoDialog(QtGui.QDialog):
    def __init__(self,tname='m12_pro',db=None,idv=None, parent=None):
    ...
    def __makeControls(self):
        ...
        fields =  utils_db.parseDDL(self.table,self.db)
        ...
        for line in fields:
            ...
            if name == 'orar' and self.table == 'm12_orar':
                typ = 'wdy'
            self.lb.append(utils_db.getLabelFromField(name,self.db))
            if typ == 'key':
    ...
    def __populate(self):
        ...
        rowVal = utils_db.getDbOneRow(self.selectSQL(), self.db)
        i=0
        if rowVal:
            for key in self.od:
                self.od[key].setVal(rowVal[i])
                i += 1
    ...
    def updateOrInsert(self):
        ...
        if self.id:
            ...
            sql = self.insertSQL()
            #Make insert Here
        insNo = utils_db.commitToDb(sql, self.db)
        if insNo:
    ...
    def updateSQL(self):
        ...
        for  key in self.od:
            sqlUpdate += "%s='%s'," % (key,self.od[key].getVal())
...
if __name__ == '__main__':
    ...
    adds = autoDialog('m12_apo','c:/ted/testing.m13',1)
    #adds = ButtonLineEdit()
    adds.show()
