...
def makeMis(xrid,perid,mistid,dat,db):
    ...
    sql2 = "INSERT INTO m12_misd(mis_id,pro_id,mtyp_id,val) VALUES(%s,%s,%s,%s)" 
    ...
    for col in par:
        ...
        #meres = 0
        if mistid == 1:
            meres = d(nullToZero(col[4])+nullToZero(col[5]))
        elif mistid == 4:
            meres = d(d(col[4]) / d(8),2)
        elif mistid == 5:
            meres = d( d(col[4]) / d(10) ,2) #Mesos oros imeron mexri maio kai meta dia dyo
        elif mistid == 3:
            meres = d(d(2)/d(19) * d(col[4]),2)
        else:
            meres = d(0)
        ...
        vals[900] = plir
        for k in vals:
            curd.execute(sql2 % (mis_id,pro_id,k,vals[k]))
    con.commit()
    cur.close()
    curd.close()
    con.close()
...
def tst():
    par = getParousies(2,1,'c:/ted/mistest.m13')
    for row in par:
        for col in row:
            print col,
        print ''   
