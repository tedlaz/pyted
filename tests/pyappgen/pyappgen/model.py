...
def gLblArray(fieldNames): # Μάλλον να μεταφερθεί στο model
    tmp = []
...
class tbl():
    ...
    def add(self, tname, lbs, lbp, repFlds=None, uni=None,):
        ...
        self.reprFields[tname] = repFlds
        self.uniqueFields[tname] = uni
    ...
    def _rfieldsArray(self,tname):
        ...
        if tables.reprFields[tname]:
            repFields = [fld.strip() for fld in tables.reprFields[tname].split(',')]
        else:
            repFields = tables.fields[tname]
        return repFields
    
    def reprFieldSql(self,table,id):
        txtSql = u"SELECT %s.id," % table
        if tables.reprFields[table]:
            repFields = tables.reprFields[table].split(',')
        else:
...
if __name__ == '__main__':
    tables.add('tst','Testing','Testing Fields')
    ...
    print startSql()
    print tables.reprFieldSql('tst',2)