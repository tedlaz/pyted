...
def coText(db):
    ...
    coname = dbu.getDbSingleVal(sql, ('1',), db)
    if coname:
...
class fMain(QtGui.QMainWindow):
    ...
    def addPlugins(self):
        self.menuArray = models.fillTreeMenu()
    ...
    def runPlugin(self):
        ...
        if self.pfiles[selection][1] == 'tblmd':
            table = self.pfiles[selection][0]
            dlg = qtGrids.masterDetailGridDlg(table,self.db,self)
            #dlg = qtGrids.gridDlg(table,self.db,self)
    ...
    def coData(self):
        frm = dbf.autoDialog(1,'co',self.db,self)
        frm.exec_()
        self.setwinTitle() 
                        
    def closeDat(self):
        frm = dbf.autoDialog(1,'lk',self.db,self)
        frm.exec_()
