...
# Check if PIL is available (tries importing both pypi version
# and corrected or manually installed versions).
...
from fpdf_util import substr, sprintf, UTF8ToUTF16BE, UTF8StringToArray
...
def page_format_tuple(format_name):
    '''
    returns format tuple.
    Default is A4 format
    '''
    rval = (595.28, 841.89)
    if isinstance(format_name, basestring):
        ...
        if format_name == 'a3':
            ...
            rval = (612.00, 792.00)
        elif format_name == 'legal':
            rval = (612.00, 1008.00)
    return rval


def set_orientation_vals(orientation, val1, val2):
    '''
    Return always values normally
    except if orientation is landscape
    '''
...
def scale_factor(unit):
    '''Scale factor from unit. Default is mm'''
...
class FPDF(object):
    ...
    def __init__(self, orientation='P', unit='mm', page_format='A4'):
        ...
        self.k = scale_factor(unit)
        # Page format
        self.fw_pt, self.fh_pt = page_format_tuple(page_format)
        self.fw = self.fw_pt / self.k
        self.fh = self.fh_pt / self.k
        # Page orientation
        self.w_pt, self.h_pt, self.def_orientation = set_orientation_vals(
            orientation, self.fw_pt, self.fh_pt)
    ...
    def linew(self, x1, y1, x2, y2, width=0.0):
        self.set_line_width(width)
        self.line(x1, y1, x2, y2)

    def line_horizontal(self, yval, x1, x2, width=0.0):
        self.linew(x1, yval, x2, yval, width=width)

    def line_vertical(self, xval, y1, y2, width=0.0):
        self.linew(xval, y1, xval, y2, width=width)
    ...
    def rectw(self, x, y, w, h, style='', width=0.0):
        self.set_line_width(width)
    ...
    def add_font(self, family, style='', fname='', uni=False):
        ...
        if uni:
            ...
            if os.path.exists(fname):
                ttffilename = fname
            elif FPDF_FONT_DIR and os.path.exists(
                    os.path.join(FPDF_FONT_DIR, fname)):
                ttffilename = os.path.join(FPDF_FONT_DIR, fname)
            elif SYSTEM_TTFONTS and os.path.exists(
                    os.path.join(SYSTEM_TTFONTS, fname)):
    ...
    def link(self, x, y, w, h, link):
        ...
        self.page_links[self.page] += [(x * self.k,
                                        self.h_pt - y * self.k,
                                        w * self.k,
                                        h * self.k,
                                        link), ]
    ...
    def rotate(self, angle, x=None, y=None):
        ...
        if angle != 0:
            ...
            cx = x * self.k
            cy = (self.h - y) * self.k
            sta1 = 'q %.5F %.5F %.5F %.5F %.2F %.2F cm 1 0 0 1 %.2F %.2F cm'
            s = sprintf(sta1, c, s, -s, c, cx, cy, -cx, -cy)
            self._out(s)
    ...
    def cell(self, w, h=0, txt='', border=0, ln=0, align='', fill=0, link=''):
        ...
        h: height
        txt: text
        border: 1 if rectangle has border 0 otherwise
        ln: ??
        align: (L)eft (C)enter (R)ight
        fill: 1 for fill 0 for no fill
        link: ??
        '''
        txt = self.normalize_text(txt)
        k = self.k
        ...
        if txt != '':
            ...
            if self.ws and self.unifontsubset:
                ...
                par1 = 'BT 0 Tw %.2F %.2F Td ['
                par2 = (self.x + dx) * k
                par3 = (self.h - (self.y + 0.5 * h + 0.3 * self.font_size)) * k
                s += sprintf(par1, par2, par3)
                t = txt.split(' ')
                numt = len(t)
                for i in range(numt):
                    tx = t[i]
            ...
            if self.underline:
                s += ' ' + self._dounderline(
                    self.x + dx, self.y + .5 * h + .3 * self.font_size, txt)
            if self.color_flag:
                s += ' Q'
            if link:
                self.link(
    ...
    def multi_cell(self,
        ...
        s = txt.replace("\r", '')  # Remove carriage returns
        nb = len(s)
        if nb > 0 and s[nb - 1] == "\n":
            nb -= 1
        b = 0
    ...
    def set_txt(self, line, xpos, text, width, align, border=0):
        self.set_xy(xpos, line)
        self.cell(ln=0, h=5.0, align=align, w=width, txt=text, border=border)
    ...
    def image(self, name, x=None, y=None, w=0, h=0, type='', link=''):
        ...
        if name not in self.images:
            ...
            if type == '':
                pos = name.rfind('.')
                if not pos:
                    erp = 'image file has no extension no type was specified: '
                    self.error(erp + name)
            ...
            if type == 'jpg' or type == 'jpeg':
                ...
                parsing_functions = [self._parsejpg,
                                     self._parsepng,
                                     self._parsegif]
        ...
        if x is None:
            x = self.x
        fstr = 'q %.2f 0 0 %.2f %.2f %.2f cm /I%d Do Q'
        self._out(sprintf(
            fstr,
            w * self.k,
            h*self.k,
        ...
        for n in xrange(1, nb + 1):
            ...
            if self.page_links and n in self.page_links:
                ...
                for pl in self.page_links[n]:
                    ...
                    annots += rect + '] /Border [0 0 0] '
                    if isinstance(pl[4], basestring):
                        annots += '/A <</S /URI /URI '
                        annots += self._textstring(pl[4]) + '>>>>'
                    else:
                        l = self.links[pl[4]]
            ...
            if self.pdf_version > '1.3':
                vou = '/Group <</Type /Group /S /Transparency /CS /DeviceRGB>>'
                self._out(vou)
    ...
    def _putfonts(self):
        ...
        for diff in self.diffs:
            # Encodings
            self._newobj()
            vl = '<</Type /Encoding /BaseEncoding /WinAnsiEncoding /Differences ['
            self._out(vl + self.diffs[diff] + ']>>')
            self._out('endobj')
                ...
                if not compressed and 'length2' in info:
                    ...
                    if header and ord(font[info['length1']]) == 128:
                        ...
                        font = substr(
                            font,
                            0,
                            info['length1']) + substr(font, info['length1'] + 6)
        ...
        for k, font in self.fonts.iteritems():
            ...
            if type == 'core':
                ...
                # CIDFontType2
                # A CIDFont whose glyph descriptions
                # are based on TrueType font technology
                self._newobj()
                self._out('<</Type /Font')
    ...
    def _putTTfontwidths(self, font, maxUni):
        ...
        for k, ws in sorted(range_.items()):
            cws = len(ws)
            if k == nextk and not prevint and (k not in range_interval or cws < 3):
    ...
    def _putimage(self, info):
        if 'data' in info:
            ...
            if info['cs'] == 'Indexed':
                tstr = '/ColorSpace [/Indexed /DeviceRGB '
                tstr += str(len(info['pal']) / 3 - 1)
                tstr += ' ' + str(self.n + 1) + ' 0 R]'
                self._out(tstr)
            else:
            ...
            # Soft mask
            if 'smask' in info:
                dps = '/Predictor 15 /Colors 1 /BitsPerComponent 8 /Columns '
                dp = dps + str(info['w'])
                smask = {'w': info['w'],
                         'h': info['h'],
                         'cs': 'DeviceGray',
    ...
    def _putinfo(self):
        ...
        self._out('/CreationDate ' + self._textstring('D:' + now))
    ...
    def _putcatalog(self):
        ...
        if self.zoom_mode == 'fullpage':
            ...
            tst2 = '/OpenAction [3 0 R /XYZ null null '
            self._out(tst2 + (self.zoom_mode/100) + ']')
    ...
    def _parsejpg(self, filename):
        ...
        try:
            ...
            msg1 = 'Missing or incorrect image file: %s. error: %s'
            self.error(msg1 % (filename, str(e)))
        else:
            a = im.size
        # We shouldn't get into here, as Jpeg is RGB=8bpp right(?),
        # but, just in case...
        bpc = 8
        if im.mode == 'RGB':
            colspace = 'DeviceRGB'
        ...
        if ct >= 4:
            ...
            if ct == 4:
                ...
                for i in range(h):
                    ...
                    color += re.sub(
                        '(.).', lambda m: m.group(1), line, flags=re.DOTALL)
                    alpha += re.sub(
                        '.(.)', lambda m: m.group(1), line, flags=re.DOTALL)
            else:
                # RGB image
                ...
                for i in range(h):
                    ...
                    line = substr(data, pos + 1, length)
                    color += re.sub(
                        '(.{3}).', lambda m: m.group(1), line, flags=re.DOTALL)
                    alpha += re.sub(
                        '.{3}(.)', lambda m: m.group(1), line, flags=re.DOTALL)
            del data
