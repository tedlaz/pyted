...
Abstract = """This paper describes και εδώ βάζουμε Ελληνικά για να δούμε την
υποστήριξη των γλωσσών που υπάρχει ενσωματωμένη στο σύστημα"""
...
def myFirstPage(canvas, doc):
    ...
    canvas.drawString(50, PAGE_HEIGHT-108, Title)
...
def go():
    Elements.insert(0, rlp.Spacer(0,inch))
    doc = rlp.SimpleDocTemplate('gfe.pdf')
...
def header(txt, style=HeaderStyle, klass=rlp.Paragraph, sep=0.3):
    s = rlp.Spacer(0.2*inch, sep*inch)
    Elements.append(s)
...
def pre(txt):
    s = rlp.Spacer(0.1*inch, 0.1*inch)
    Elements.append(s)
    p = rlp.Preformatted(txt, PreStyle)
    Elements.append(p)
