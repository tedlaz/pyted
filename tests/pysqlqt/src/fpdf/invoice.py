...
class Invoice():
    ...
    def make(self):
        ...
        for i, el in enumerate(self.data['lines']):
            ...
            self.pdf.set_txt(line, 111.0, dec.strGrDec(el['pos']), 17.0, 'R', 0)
            self.pdf.set_txt(line, 130.0, dec.strGrDec(el['timi']), 17.0, 'R', 0)
    ...
    def print_header(self):
        ...
        self.pdf.set_txt(55.0, 130.0, ': %s' % self.data['inum'], 20.0, 'L', 0)
        self.pdf.set_txt(60.0, 101.0, u'Ημερομηνία - ώρα', 25.0, 'L', 0)
        self.pdf.set_txt(60.0, 130.0, ': %s' % self.data['idat'], 23.0, 'L', 0)
        self.pdf.set_txt(65.0, 101.0, u'Τρόπος πληρωμής', 20.0, 'L', 0)
        self.pdf.set_txt(65.0, 130.0, ': %s' % self.data['ipli'], 40.0, 'L', 0)
        self.pdf.set_txt(70.0, 101.0, u'Σκοπός διακίνησης', 20.0, 'L', 0)
        self.pdf.set_txt(70.0, 130.0, ': %s' % self.data['isko'], 40.0, 'L', 0)
        self.pdf.set_txt(75.0, 101.0, u'Τόπος αποστολής', 20.0, 'L', 0)
        self.pdf.set_txt(75.0, 130.0, ': %s' % self.data['iapo'], 40.0, 'L', 0)
        self.pdf.set_txt(80.0, 101.0, u'Τόπος προορισμού', 20.0, 'L', 0)
        self.pdf.set_txt(80.0, 130.0, ': %s' % self.data['ipro'], 40.0, 'L', 0)
        ...
        self.pdf.set_txt(22, 35.0, u'Α.Φ.Μ.: %s - Δ.Ο.Υ.: %s' % (self.data['cafm'], self.data['cdoy']), 155.0, 'C', 0)
        ...
        self.pdf.set_font('fnt', '', 10.0)
        self.pdf.set_txt(50.0, 10.0, u'Επωνυμία', 20.0, 'L', border=0)
        self.pdf.set_txt(50.0, 35.0, ': %s' % self.data['pepo'], 60.0, 'L', border=0)
        self.pdf.set_txt(54.0, 10.0, u'Α.Φ.Μ.', 20.0, 'L', border=0)
        self.pdf.set_txt(54.0, 35.0, ': %s' % self.data['pafm'], 20.0, 'L', border=0)
        self.pdf.set_txt(58.0, 10.0, u'Δ.Ο.Υ.', 20.0, 'L', border=0)
        self.pdf.set_txt(58.0, 35.0, ': %s' % self.data['pdoy'], 20.0, 'L', border=0)
        self.pdf.set_txt(62.0, 10.0, u'Επάγγελμα', 20.0, 'L', border=0)
        self.pdf.set_txt(62.0, 35.0, ': %s' % self.data['pepa'], 60.0, 'L', border=0)
        self.pdf.set_txt(66.0, 10.0, u'Διεύθυνση', 18.0, 'L', border=0)
        self.pdf.set_txt(66.0, 35.0, ': %s' % self.data['pado'], 70.0, 'L', border=0)
        self.pdf.set_txt(70.0, 10.0, u'Πόλη - Τ.Κ.', 18.0, 'L', border=0)
        self.pdf.set_txt(70.0, 35.0, ': %s' % self.data['padp'], 22.0, 'L', border=0)
        self.pdf.set_txt(74.0, 10.0, u'Τηλέφωνο', 18.0, 'L', border=0)
        self.pdf.set_txt(74.0, 35.0, ': %s' % self.data['ptel'], 30.0, 'L', border=0)
    ...
    def print_page_footer(self, cpage):
        ...
        self.pdf.set_txt(
            row,
            50,
            u'Σελίδα %s από %s σε μεταφορά' % (cpage, self.total_pages), 20.0, 'R', 0)
...
if __name__ == '__main__':
    dati = [{'per': 'Δοκιμή', 'mon': 'Τεμ', 'pos': 2, 'timi': 10, 'synt': '13'},
            {'per': 'Δοκιμή2', 'mon': 'Τεμ', 'pos': 1, 'timi': 10, 'synt': '10',},
            {'per': 'Λογιστικές υπηρεσίες', 'mon': 'Τεμ', 'pos': 1, 'timi': 100, 'synt': '23'}]
    dat = {'name': 'test',
