...
import qt_auto_form as qaf

import dbutils as dbu
import qt_about as about
import dbmodel as model
import qt_grids as qg
import res_rc
...
def coText(db):
    ...
    '''
    sql = "SELECT scop || ' ' || scono FROM co WHERE id=1"
    rws, columnNames, status, msg = dbu.dbRows(sql, db, 1)
    coname = rws[0][0]
    if coname:
        return u"%s" % coname.strip()
    else:
...
class fMain(QtGui.QMainWindow):
    ...
    def __init__(self, app_config,parent=None):
        ...
        self.exStatus()
        self.setWindowTitle(self.app_config.APPLIC_TITLE)
    ...
    def createActions(self):
        ...
        #self.closeDatAct = QtGui.QAction(QtGui.QIcon(':/images/par.png'),u"Ημ/νία κλειδώματος", self,
        #                             statusTip=u"Μόνο μετά από αυτή την ημερομηνία μπορούν να γίνουν εγγραφές",triggered=self.closeDat)

        self.coDataAct = QtGui.QAction(QtGui.QIcon(':/images/set.png'),u"Στοιχεία επιχείρησης", self, 
                                     statusTip=u"Στοιχεία επιχείρησης",triggered=self.coData)                        
    ...
    def addPlugins(self):
        self.menuArray = model.fillTreeMenu()
    ...
    def runPlugin(self):
        ...
        if self.pfiles[selection][1] == 'tbl':
            ...
            dlg = qg.gridDlg(table,self.db,self)

        if self.pfiles[selection][1] == 'tblmd':
            table = self.pfiles[selection][0]
            dlg = qg.gridDlgMD(table,self.db,self)
            #dlg = qtGrids.gridDlg(table,self.db,self)
            ...
            #dlg =dbu.fsqlData(sql,self.db,inPanel=True,wtitle=self.pfiles[selection][3])
            pass
    ...
    def newDB(self):
        ...
        fname = '%s' %QtGui.QFileDialog.getSaveFileName(self,
                u"%s Δημιουργία Νέου αρχείου" % self.app_config.APPLIC_TITLE,
                os.path.dirname(self.db),
                u"Αρχείο (*.%s)" % self.app_config.DB_EXTENSION)
        sql = model.startSql()
        
        if fname:
            dbu.dbScript(sql,fname )
            self.resetStackw()
            self.db = fname
    ...
    def fileOpen(self):
        fname = QtGui.QFileDialog.getOpenFileName(self, u'Επιλογή Αρχείου',self.db,u"*.%s" % self.app_config.DB_EXTENSION) 
        if fname:
            self.resetStackw()
            self.db = '%s' % fname
            self.settings.setValue("db_path",self.db)#QtCore.QVariant(self.db))
    ...
    def onStartOrFileOpen(self):
        if os.path.isfile(self.db):
            if isValidDB(self.db):
                ...
                QtGui.QMessageBox.critical(self, u'%s - Πρόβλημα !!!' % self.app_config.APPLIC_TITLE, u'Λανθασμένο αρχείο')
                #self.ui.treeMis.setModel(None)
                self.db = ''
                self.setWindowTitle(u"%s -> ( ??? )" % self.app_config.APPLIC_TITLE)
                self.resetStackw()
                return False
        else:
            #self.ui.treeMis.setModel(None)
    ...
    def setwinTitle(self):
        self.setWindowTitle(u"%s -> %s ( %s )" %(self.app_config.APPLIC_TITLE, coText(self.db), self.db)) 
                                    
    def about(self):
        frm = about.dlg(txt=self.app_config.APPLIC_TITLE,about=self.app_config.ABOUT , parent=self)
        frm.exec_()
    ...
    def coData(self):
        frm = qaf.autoForm(1,'co',self.db,self)
        frm.exec_()
        self.setwinTitle()
                        
    def closeDat(self):
        frm = qaf.autoForm(1,'lk',self.db,self)
        frm.exec_()

def main(app_config):
    app = QtGui.QApplication(sys.argv)
    app.setOrganizationName(app_config.ORG_NAME)
    app.setOrganizationDomain(app_config.ORG_DOMAIN)
    app.setApplicationName(app_config.APPLIC_NAME)
    myApp = fMain(app_config)
    myApp.show()
    sys.exit(app.exec_())
...
def testRun():
    ...
    a = ac()
    a.ORG_NAME = 'tstOrg'
    a.ORG_DOMAIN = 'tstDomain'
    a.APPLIC_NAME = 'tstApp'
    a.APPLIC_TITLE = 'tstApplicationTitle'
    a.ABOUT = 'HtmlAbout'
    a.DB_EXTENSION = 'tst'
    main(a)
                       
if __name__ == "__main__":
    import test_create_demo_db as tcdd
    testRun()